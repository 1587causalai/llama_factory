#!/usr/bin/env python
# coding=utf-8
"""测试使用 UltraFeedback 数据集进行 Beta DPO 训练"""
...
def test_betadpo_ultrafeedback():
    print_section("Beta DPO with UltraFeedback Dataset Testing")
    ...
    config_file = os.path.join(os.path.dirname(__file__), "configs", "test_betadpo_ultrafeedback.yaml")
    ...
    config_table.add_row("BetaHead 类型", str(finetuning_args.beta_head_type))
    config_table.add_row("BetaHead epsilon", str(finetuning_args.beta_head_epsilon))
    
    rprint(config_table)
    
    # 3. 运行 Beta DPO 训练
    print_section("2. Beta DPO 训练开始")
    ...
    try:
        ...
        rprint("[green]Beta DPO 训练完成![/green]")
    except Exception as e:
        rprint("[red]训练过程中发生错误:[/red]")

...
def test_dpo_qwen_step1_config():
    """第一步：测试配置加载"""
    try:
        print("=== 步骤 1: 测试配置加载 ===")
...
def test_dpo_qwen_step2_model(model_args: ModelArguments):
    """第二步：测试模型加载"""
    try:
        print("\n=== 步骤 2: 测试模型加载 ===")
...
def test_dpo_qwen_step3_dataset(
...
def test_dpo_qwen_step4_training(
    model_args: ModelArguments,
    data_args: DataArguments,
    training_args: "Seq2SeqTrainingArguments",
    finetuning_args: FinetuningArguments
...
def main():
    """主测试函数"""
    print("开始 Qwen1.5-0.5B-Chat DPO 渐进式测试...")
    
    # 步骤 1: 配置加载
    config_result = test_dpo_qwen_step1_config()
    ...
    # 步骤 2: 模型加载
    model_result = test_dpo_qwen_step2_model(model_args)
    ...
    # 步骤 3: 数据集加载
    dataset_result = test_dpo_qwen_step3_dataset(
        model_args=model_args,
        data_args=data_args,
        training_args=training_args,
        finetuning_args=finetuning_args
    ...
    # 步骤 4: 训练流程
    training_result = test_dpo_qwen_step4_training(
        model_args=model_args,
        data_args=data_args,
        training_args=training_args,
        finetuning_args=finetuning_args

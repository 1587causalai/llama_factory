"""
测试 Learnable Beta DPO 的前向传播和损失计算
"""
import os
import sys
import yaml
...
def test_forward_pass():
    ...
    print_section("Learnable Beta DPO Forward Pass Testing")
    ...
    with Timer("配置加载"):
        ...
        config_table.add_row("Beta Head Type", str(finetuning_args.beta_head_type))
        config_table.add_row("Beta Head Epsilon", str(finetuning_args.beta_head_epsilon))
    ...
    with Timer("训练器初始化"):
        ...
        # 2) 初始化训练器
        trainer = LearnableBetaDPOTrainer(
            model=model,
            ref_model=None,
            args=training_args,
            finetuning_args=finetuning_args,
            data_collator=data_collator,
            **dataset_module,  # 这里包含了 train_dataset
            **tokenizer_module,  # 这里包含了 tokenizer 和 processor
        )
    ...
    with Timer("前向传播计算"):
        try:
            ...
            # 打印每个输出的形状和值
            chosen_logps, rejected_logps, chosen_logits, rejected_logits, chosen_logps_avg, betas, ppls = outputs
            ...
            output_table.add_row("betas", str(betas.shape), f"{betas.mean().item():.4f}")
            output_table.add_row("ppls", str(ppls.shape), f"{ppls.mean().item():.4f}")

...
6. **GRPO (Group Relative Policy Optimization)** - 群组相对策略优化
   - 文件：[grpo_overview.md](grpo_overview.md)
   - 状态：✅ 已完成
   - 注意：此算法在LLaMA Factory的相关项目EasyR1中实现，详见[LLaMA Factory与EasyR1关系说明](llama_factory_and_easyr1.md)
...
- [LLaMA Factory与EasyR1项目关系说明](llama_factory_and_easyr1.md) - 解释LLaMA Factory和EasyR1项目的关系以及GRPO算法的应用
...
### GRPO (群组相对策略优化)
- **特点**：PPO的改进版，使用群组相对优势估计代替值函数网络
- **优势**：比PPO更高效稳定，内存需求更低，计算更简化
- **劣势**：实现较复杂，需要组批量生成和评估
- **适用场景**：
  - 大规模模型训练（如超过7B参数）
  - 有奖励模型但计算资源受限
...
2. **追求最佳性能**：可考虑SFT→DPO/KTO→PPO/GRPO的渐进式训练
3. **特定领域适应**：PT→SFT的组合可能效果最佳
4. **对齐任务**：
   - 轻度对齐：SFT或DPO
   - 中度对齐：KTO
   - 强度对齐：PPO或GRPO
5. **训练稳定性优先**：SFT > KTO > GRPO > DPO > PPO
6. **大规模模型训练**：GRPO > KTO > DPO > PPO
